<template>
  <div class="body">
    <main>
      <div class="wrapper">
        <div>
          <h2 class="control-panel_title">Servicios</h2>
        </div>
        <div>
          <NuxtLink to="/admin/servicios/create" class="btn btn--blue">
            Crear servicio
          </NuxtLink>
        </div>
        <table>
          ...
          <tbody>
            <tr v-for="service of services" :key="service.recId">
              <td>{{ service.title }}</td>
              <td>{{ service.description }}</td>
              <td>{{ service.category }}</td>
              <td>
                <NuxtLink :to="'/admin/servicios/' + service.recId">
                  Editar
                </NuxtLink>
              </td>
              <td>
                <button @click="openModal(service.recId)">Borrar</button>
              </td>
              <ModalDelete
                :id="service.recId"
                @delete="deleteElement(service.recId)"
                @close="modalClose(service.recId)"
              />
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    deleteElement(id) {
      this.$store.dispatch('service/deleteService', { doc: id })
      this.modalClose()
    },
    modalClose(id) {
      this.$modal.hide(id)
