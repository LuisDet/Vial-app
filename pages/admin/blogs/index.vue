<template>
  <div class="body">
    <main>
      <div class="wrapper">
        <div>
          <h2 class="control-panel_title">Blogs</h2>
        </div>
        <div>
          <NuxtLink to="/admin/blogs/create" class="btn btn--blue">
            Crear blog
          </NuxtLink>
        </div>
        <table>
          ...
          <tbody>
            <tr v-for="blog of blogs" :key="blog.recId">
              <td>{{ blog.title }}</td>
              <td>{{ blog.author }}</td>
              <td>{{ blog.date }}</td>
              <td>
                <NuxtLink :to="'/admin/blogs/' + blog.recId"> Editar </NuxtLink>
              </td>
              <td>
                <button @click="openModal(blog.recId)">Borrar</button>
              </td>
              <ModalDelete
                :id="blog.recId"
                @delete="deleteElement(blog.recId)"
                @close="modalClose(blog.recId)"
              />
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  async mounted() {
    if (this.blogs.length === 0) await this.$store.dispatch('blog/setBlogs')
  },
  methods: {
    openModal(id) {
      this.$modal.show(id)
